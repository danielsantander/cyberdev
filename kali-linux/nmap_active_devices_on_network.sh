...
if [ -z "$1" ]
  then
    cidr_range=$(ip --brief address show | awk '$3 ~ /^[0-9]{1,3}(\.[0-9]{1,3}){3}\/[0-9]{2}$/ {print $3}')
else
    cidr_range=$1;
fi
...
if [ -z ${cidr_range+x} ]
then
    echo "ip address range ($cidr_range) is unset, exiting.";
    exit 1;
else
    echo "performing nmap on $cidr_range ..."

    network=$(nmap -sP $cidr_range | awk '$NF ~ /[0-9]+(\.[0-9]+){3}/ {print $NF}')
